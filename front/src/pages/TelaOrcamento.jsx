...
import { orcamentoApi } from '../services/api.migration';
...
const TelaOrcamento = () => {
  ...
  const carregarPlanos = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 'success' && response.data) {
        const { orcamentos: listaPlanos, pagination: dadosPaginacao } = response.data;
        ...
        const planosProcessados = listaPlanos.map(plano => ({
          ...
          valorFormatado: formatarValor(plano.totalReceita || 0),
          criadoEmFormatado: new Date(plano.createdAt).toLocaleDateString('pt-AO'),
          ano: plano.ano || new Date(plano.createdAt).getFullYear()
        }));
        
        // Ordenar por status: aprovados primeiro, depois por data de criação
        const planosOrdenados = planosProcessados.sort((a, b) => {
          // Primeiro: aprovados
        ...
        setPaginacao({
          totalItens: dadosPaginacao.total,
          totalPaginas: dadosPaginacao.totalPaginas,
          paginaAtual: dadosPaginacao.pagina
        });
        
        console.log('✅ Planos carregados:', planosOrdenados);
      } else {
  ...
  }, [carregarPlanos, carregarEstatisticas]);
  ...
  const handleVisualizarPlano = (planoId) => {
    // Navegar para página de detalhes do plano
    navigate(`/orcamento/${planoId}`);
  };

  const handleEditarPlano = (planoId) => {
    // Navegar para página de edição do plano
    navigate(`/orcamento/${planoId}/editar`);
  };

  const handleBaixarPlano = async (planoId) => {
    try {
      console.log('📄 Baixando plano:', planoId);
      const response = await orcamentoApi.exportarOrcamento(planoId, 'pdf');
      ...
      link.href = url;
      link.download = `plano-${planoId}.pdf`;
          ...
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <h1 className="text-2xl font-bold bg-gradient-to-b from-white to-gray-200 bg-clip-text text-transparent">GESTÃO DE PLANOS</h1>
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl shadow-black/30 overflow-hidden">
            <div className="p-6 border-b border-white/10 bg-gradient-to-r from-white/5 to-white/10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-xl font-bold text-white">Meus Planos</h2>
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-white/70 mb-6">
                    {filtros.busca || filtros.status 
                      ...
                      : 'Crie seu primeiro plano para começar a planejar suas finanças'
                    }
                  </p>
                  {!filtros.busca && !filtros.status && (
                    <button
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative">
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                              <div>
                                ...
                                <p className="font-bold text-white">{plano.valorFormatado}</p>
                              </div>
                              <div>
                                <p className="text-white/60">Criado em</p>
                                <p className="font-bold text-white/90">{plano.criadoEmFormatado}</p>
                              </div>
                              <div>
                                <p className="text-white/60">Ano</p>
                                <p className="font-bold text-blue-400">{plano.ano}</p>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="flex gap-2">
                              ...
                              {(plano.status !== 'arquivado' && plano.status !== 'aprovado') && (
                                <button 
